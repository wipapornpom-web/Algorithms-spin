...
const games = [
  {id:'brush', icon:'🦷', title:'ภารกิจแปรงฟัน', instruction:'🤖 จีบนิ้วจับรูป แล้วลากวางตามลำดับการแปรงฟัน', type:'sequence', steps:['🪥','🧴','😁','💧']},
  {id:'pizza', icon:'🍕', title:'ภารกิจทำพิซซ่า', instruction:'🤖 ลากรูปขั้นตอนทำพิซซ่าให้ถูกลำดับ', type:'sequence', steps:['🥖','🍅','🧀','🔥']},
  {id:'rocket', icon:'🚀', title:'ภารกิจปล่อยจรวด', instruction:'🤖 ลากภาพขั้นตอนปล่อยจรวดให้ถูกลำดับ', type:'sequence', steps:['🔧','⛽','🔢','🚀']},
  {id:'plant', icon:'🌱', title:'ภารกิจปลูกต้นไม้', instruction:'🤖 เรียงภาพขั้นตอนปลูกต้นไม้ให้ถูกต้อง', type:'sequence', steps:['🕳️','🌱','🪣','🌳']},
  {id:'computer', icon:'🖥️', title:'ภารกิจเปิดคอมพิวเตอร์', instruction:'🤖 ลากภาพขั้นตอนเปิดคอมพิวเตอร์ตามลำดับ', type:'sequence', steps:['🔌','🔘','🖥️','⌨️']},
  {id:'hands', icon:'👐', title:'ภารกิจล้างมือ', instruction:'🤖 เรียงภาพขั้นตอนล้างมือ', type:'sequence', steps:['🚰','🧼','👐','🧻']},
  {id:'toy', icon:'🧸', title:'ภารกิจเก็บของเล่น', instruction:'🤖 ลากของเล่นใส่กล่องให้ครบ', type:'collect', robot:'🧸', targets:['📦','📦','📦']},
  {id:'diamond', icon:'💎', title:'ภารกิจเก็บเพชร', instruction:'🤖 ลากหุ่นยนต์ไปเก็บเพชร แล้วไปถึงธง', type:'map', robot:'🤖', targets:['💎','💎','🏁']},
  {id:'flag', icon:'🏁', title:'ภารกิจไปหาธง', instruction:'🤖 ลากหุ่นยนต์ตามทางไปถึงธง', type:'map', robot:'🤖', targets:['⭐','🏁']},
  {id:'school', icon:'🏫', title:'ภารกิจไปโรงเรียน', instruction:'🤖 ลากนักเรียนผ่านหนังสือ แล้วไปโรงเรียน', type:'map', robot:'🧒', targets:['📘','🏫']}
];
...
/** สร้างเกมลากตัวละครผ่านเป้าหมาย */
function renderMap(game){
  const area = playArea.getBoundingClientRect();
  const robot = document.createElement('div'); robot.className='mapRobot draggable mapActor'; robot.textContent=game.robot; robot.dataset.value='actor';
  robot.style.left = '55px'; robot.style.top = `${area.height-160}px`;
  playArea.appendChild(robot);
  const positions = game.targets.map((t,i)=>({
    emoji:t,
    x: 210 + i * Math.min(230, (area.width-360)/Math.max(1,game.targets.length-1)),
    y: i % 2 === 0 ? 80 : area.height-185
  }));
  positions.forEach((p,i)=>{
    const line = document.createElement('div'); line.className='pathLine';
    line.style.left = `${i===0 ? 120 : positions[i-1].x+70}px`; line.style.top = `${p.y+55}px`; line.style.width = `${Math.max(100,p.x-(i===0?120:positions[i-1].x+70))}px`;
    playArea.appendChild(line);
    const z = document.createElement('div'); z.className='mapTarget dropzone'; z.dataset.accept=p.emoji; z.textContent=p.emoji;
    z.style.left=`${p.x}px`; z.style.top=`${p.y}px`; z.style.width='120px'; z.style.height='120px';
...
function endDrag(x,y){
  ...
  if(zone){
    if(currentGame?.type === 'map'){
      zone.dataset.visited='yes'; zone.style.background='rgba(120,255,170,.65)'; zone.style.transform='scale(.92)';
      dragging.style.left = `${zone.getBoundingClientRect().left+5}px`; dragging.style.top = `${zone.getBoundingClientRect().top+5}px`;
    }else{
